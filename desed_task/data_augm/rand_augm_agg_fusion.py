...
class RandAugment:
    def __init__(self,
        ...
        self.indx_synth, self.indx_weak, self.indx_unlabelled = hparams["training"]["batch_size"]
        ...
        strong_mask[:self.indx_synth] = 1
        weak_mask[self.indx_synth: self.indx_weak + self.indx_synth] = 1
        unsup_mask[self.indx_weak + self.indx_synth:] = 1
        self.weak_mask = weak_mask.bool()
        self.strong_mask = strong_mask.bool()
        self.unsup_mask = unsup_mask.bool()

        # Get mel spectrum extractor
        self.mel_spec_crnn = self.__get_mel_spec_crnn()
        self.mel_spec_atst = self.__get_mel_spec_atst()
    ...
    def random_augment(self, batch: torch.FloatTensor, labels: torch.FloatTensor, epoch=None):
        ...
        if self.mixup_func is None and self.augment_selector is None:
            ...
            return self.mel_spec_crnn(batch), labels, masks, distortions
        ...
        strong_mask = torch.cat([self.strong_mask,
                                 torch.ones(self.indx_synth * 2).to(self.device),
                                 torch.zeros(self.indx_weak * 2).to(self.device)], dim=0).bool()
        weak_mask = torch.cat([self.weak_mask,
                               torch.zeros(self.indx_synth * 2).to(self.device),
                               torch.ones(self.indx_weak * 2).to(self.device)], dim=0).bool()
        # Update the augmented data and labels to the batch and labels.
        aug_batch = (torch.cat([self.mel_spec_crnn(batch),
                               aug_strong_data[0],
                               aug_weak_data[0]], dim=0),
                     torch.cat([self.mel_spec_atst(batch),
                                aug_strong_data[1],
                                aug_weak_data[1]], dim=0)
                     )
        ...
        aug_batch = (torch.cat([aug_batch[0],
                               aug_unsup_data[0]], dim=0),
                     torch.cat([aug_batch[1],
                                aug_unsup_data[1]], dim=0),
                     )
        aug_labels = torch.cat([aug_labels,
                                aug_unsup_labels], dim=0)
        # Update the masks
        weak_mask = torch.cat([weak_mask,
                               torch.zeros(self.indx_unlabelled * 2).to(self.device)], dim=0).bool()
        strong_mask = torch.cat([strong_mask,
                                 torch.zeros(self.indx_unlabelled * 2).to(self.device)], dim=0).bool()
        unsup_mask = torch.cat([self.unsup_mask,
                                torch.zeros(self.indx_synth * 2).to(self.device),
                                torch.zeros(self.indx_weak * 2).to(self.device),
                                torch.ones(self.indx_unlabelled * 2).to(self.device)], dim=0).bool()
    ...
    def _augment(self, data, label):
        ...
        if self.augment_selector is None:
            ...
            if self.mixup_func is None:
                ...
                aug_data = (torch.cat([data_mixup[0], data_randaug[0]], dim=0), torch.cat([data_mixup[1], data_randaug[1]], dim=0))
    ...
    def time_shift(self, utt: torch.FloatTensor, labels: torch.FloatTensor):
        ...
        with torch.no_grad():
            ...
            spec_crnn = self.mel_spec_crnn(utt_aug)
            spec_atst = self.mel_spec_atst(utt_aug)
        return (spec_crnn, spec_atst), labels_aug, ("Time shift", frame_split)
    ...
    def time_mask(self, utt: torch.FloatTensor, labels: torch.FloatTensor):
        ...
        with torch.no_grad():
            ...
            spec_crnn = self.mel_spec_crnn(utt * mask_mat)
            spec_atst = self.mel_spec_atst(utt * mask_mat)
        return (spec_crnn, spec_atst), labels, ("Time mask", None)
    ...
    def freq_mask(self, utt: torch.FloatTensor, labels: torch.FloatTensor):
        ...
        with torch.no_grad():
            bsz = utt.shape[0]
            mel_crnn = self.mel_spec_crnn(utt)
            mel_atst = self.mel_spec_atst(utt)
            mask_bands_crnn = torch.ones_like(mel_crnn).to(self.device)
            mask_bands_atst = torch.ones_like(mel_atst).to(self.device)
            scale = random.randint(1, self.augment_scale) * 2   # Maximal mask 20 dims in LogMel
            mask_band_start = np.random.choice(np.arange(mel_crnn.shape[1] - scale), size=bsz, replace=False)
            for i, start in zip(range(bsz), mask_band_start):
                mask_bands_crnn[i, start: start+scale, :] = 0
                mask_bands_atst[i, start: start+scale, :] = 0
        return (mel_crnn * mask_bands_crnn, mel_atst * mask_bands_atst), labels, ("Freq mask", None)
    ...
    def filter_aug(self, utt: torch.FloatTensor, labels: torch.FloatTensor):
        ...
        with torch.no_grad():
            db_range = (-7.5, 6)
            spec_crnn = self.mel_spec_crnn(utt)
            spec_atst = self.mel_spec_atst(utt)
            batch_size, n_freq_bin, _ = spec_crnn.shape
            ...
            band_factors = torch.rand((batch_size, n_freq_band)).to(spec_crnn) * (db_range[1] - db_range[0]) + \
                           db_range[0]

            band_factors = 10 ** (band_factors / 20).to(spec_crnn)

            freq_filt = torch.ones((batch_size, n_freq_bin, 1)).to(spec_crnn)
            for i in range(n_freq_band):
                freq_filt[:, band_bndry_freqs[i]:band_bndry_freqs[i + 1], :] = band_factors[:, i].unsqueeze(
                    -1).unsqueeze(-1)

            return (spec_crnn * freq_filt, spec_atst * freq_filt[:, :n_freq_bin // 2, :]), labels, ("Filter", None)
    ...
    def mixup_hard(self, utt: torch.FloatTensor, labels: torch.FloatTensor):
        ...
        with torch.no_grad():
            ...
            return (self.mel_spec_crnn(mixed_data), self.mel_spec_atst(mixed_data)), mixed_target, ("Hard mixup", perm_list)

    @staticmethod
    def rand_sign():
        return 1 if torch.randn(1) > 0.5 else -1

    def __get_mel_spec_crnn(self):
    ...
    def __get_mel_spec_atst(self):
        return MelSpectrogram(self.feat_params["sample_rate"],
                              f_min=60,
                              f_max=7800,
                              hop_length=160,
                              win_length=1024,
