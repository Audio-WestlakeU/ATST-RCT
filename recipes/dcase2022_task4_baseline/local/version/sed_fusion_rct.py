...
from desed_task.data_augm.rand_augm_agg_fusion import RandAugment
...
class SEDTask4_2021(pl.LightningModule):
    ...
    def __init__(
        ...
        self.test_temp = self.hparams["training"]["test_temp"]
        ...
        self.mel_spec_atst = MelSpectrogram(feat_params["sample_rate"],
                                       f_min=60,
                                       f_max=7800,
                                       hop_length=160,
                                       win_length=1024,
                                       n_fft=1024,
                                       n_mels=64)
        self.scaler_atst = MinMax(min=-79.6482, max=50.6842)
        self.mel_spec_crnn = MelSpectrogram(
        ...
        self.scaler_crnn = self._init_scaler()
        ...
        self.val_thd = self.hparams["training"]["val_thresholds"][0]
    ...
    def take_log(self, mels, atst=True):
    ...
    def detect(self, feats_atst, feats_crnn, model, temp=None):
        return model(self.scaler_atst(self.take_log(feats_atst, atst=True)), self.scaler_crnn(self.take_log(feats_crnn, atst=False)), temp=temp)
    ...
    def training_step(self, batch, batch_indx):
        ...
        features, labels, masks, distortions = self.augmentor.random_augment(audio, labels)
        indx_synth, indx_weak, indx_unlabelled = self.hparams["training"]["batch_size"]
        features_crnn = features[0]
        features_atst = features[1]
        ...
        # split the input into chunks; the model would automatically process the prediction in chunks
        if self.chunk_input:
            feat_chunk_end = features_crnn.shape[-1] // 2
            features_crnn = torch.cat([features_crnn[:, :, :feat_chunk_end], features_crnn[:, :, feat_chunk_end: -1]],
                                      dim=0)
            features_atst = torch.cat([features_atst[:, :, :feat_chunk_end], features_atst[:, :, feat_chunk_end: -1]],
                                      dim=0)

        # sed student forward
        strong_preds_student, weak_preds_student = self.detect(
            features_crnn, features_atst, self.sed_student
        )
        ...
        with torch.no_grad():
            strong_preds_teacher, weak_preds_teacher = self.detect(
                features_crnn, features_atst, self.sed_teacher
            )
    ...
    def validation_step(self, batch, batch_indx):
        ...
        features = self.mel_spec_crnn(audio)
        features_atst = self.mel_spec_atst(audio)
        if self.chunk_input:
            feat_chunk_end = features.shape[-1] // 2
            features = torch.cat([features[:, :, :feat_chunk_end], features[:, :, feat_chunk_end: -1]],
                                      dim=0)
            features_atst = torch.cat([features_atst[:, :, :feat_chunk_end], features_atst[:, :, feat_chunk_end: -1]],
                                      dim=0)

        strong_preds_student, weak_preds_student = self.detect(features, features_atst,  self.sed_student)
        # prediction for teacher
        strong_preds_teacher, weak_preds_teacher = self.detect(features, features_atst,  self.sed_teacher)
        ...
        if torch.any(mask_synth):
            ...
            for th in self.val_buffer_student_synth.keys():
                self.val_buffer_student_synth[th] = pd.concat(
                    [self.val_buffer_student_synth[th], decoded_student_strong[th]], ignore_index=True)
            ...
            for th in self.val_buffer_teacher_synth.keys():
                self.val_buffer_teacher_synth[th] = pd.concat(
                    [self.val_buffer_teacher_synth[th], decoded_teacher_strong[th]], ignore_index=True)
    ...
    def validation_epoch_end(self, outputs):
        ...
        synth_student_event_macro = log_sedeval_metrics(
            self.val_buffer_student_synth[self.val_thd], self.hparams["data"]["synth_val_tsv"],
        )[0]
        ...
        synth_teacher_event_macro = log_sedeval_metrics(
            self.val_buffer_teacher_synth[self.val_thd], self.hparams["data"]["synth_val_tsv"],
        )[0]
    ...
    def test_step(self, batch, batch_indx):
        ...
        fs = 16000
        audio_insec = audio_length / fs
        if len(audio[0]) > 160000:

            if math.ceil((audio_insec - 10) / 8):
                print("Audio in seconds:", audio_insec)
                print("Padd in seconds:", math.ceil((audio_insec - 10) / 8))
                print("Padding length:", fs * 10 + fs * 8 * math.ceil((audio_insec - 10) / 8) - audio_length)
                audio = torch.nn.functional.pad(audio, (0, fs * 10 + fs * 8 * math.ceil((audio_insec - 10) / 8) - audio_length))
                audio_length = audio.shape[1]
                print("Audio length (Padding):", len(audio[0]))

            exp_len = len(audio[0]) / fs
            sep_len = 8 * fs
            clip_len = 10 * fs
            clip_num = torch.arange((audio_length - clip_len) / sep_len + 1).to(audio)
            start_pos = sep_len * clip_num
            end_pos = start_pos + clip_len
            print("Starting positions:", start_pos)
            print("Ending positions:", end_pos)
            index_mat = torch.arange(0, audio_length).to(audio).expand(len(clip_num), audio_length)
            mask = (end_pos.unsqueeze(1) > index_mat) & (index_mat >= start_pos.unsqueeze(1))
            indexes = torch.argwhere(mask)
            row_index = indexes[:, 1]
            audio = torch.index_select(audio, dim=1, index=row_index).reshape(len(clip_num), -1)
            print("Decomposed audio:", audio.shape)
        else:
            exp_len = False
        # prediction for student
        features = self.mel_spec_crnn(audio)
        ...
        strong_preds_student, _ = self.detect(features, features_atst, self.sed_student, self.test_temp)
        # prediction for teachers
        strong_preds_teacher, _ = self.detect(features, features_atst, self.sed_teacher, self.test_temp)
        ...
        decoded_student_strong = batched_decode_preds(
            ...
            oversized=exp_len,
            audio_insec=audio_insec
        )

        for th in self.test_psds_buffer_student.keys():
            self.test_psds_buffer_student[th] = pd.concat(
                [self.test_psds_buffer_student[th], decoded_student_strong[th]], ignore_index=True)
        ...
        for th in self.test_psds_buffer_teacher.keys():
            self.test_psds_buffer_teacher[th] = pd.concat(
                [self.test_psds_buffer_teacher[th], decoded_teacher_strong[th]], ignore_index=True)
