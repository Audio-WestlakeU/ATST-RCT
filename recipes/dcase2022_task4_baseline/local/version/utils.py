...
def batched_decode_preds(
    strong_preds, filenames, encoder, thresholds=[0.5], median_filter=[7] * 10, pad_indx=None, oversized=False, audio_insec=10
):
    ...
    if oversized:
        for c_th in thresholds:
            c_preds = strong_preds
            if pad_indx is not None:
                true_len = int(c_preds.shape[-1] * pad_indx[0].item())
                c_preds = c_preds[:, true_len]
            pred = c_preds.detach().cpu().numpy() > c_th
            pred = pull_back_preds(pred, oversized)
            pred_fil = []
            median_filter = [7] * 10
            for pred_class, fil_val in zip(pred, median_filter):
                pred_mid = scipy.ndimage.filters.median_filter(pred_class[:, np.newaxis], (fil_val, 1))
                pred_fil.append(pred_mid)

            pred_fil = np.array(pred_fil).squeeze(-1).T
            encoder.set_audio_len(audio_insec)
            pred = encoder.decode_strong(pred_fil)
            pred = pd.DataFrame(pred, columns=["event_label", "onset", "offset"])
            pred["filename"] = Path(filenames[0]).stem + ".wav"
